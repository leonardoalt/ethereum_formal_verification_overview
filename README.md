...
* [EquiVM](https://github.com/argotorg/EquiVM): Foundational refinement proofs in Lean 4 establishing that a piece of EVM bytecode implements a high-level program written in Sol⁻, a small Solidity-like specification language, which gives a human-readable and compiler-agnostic formal view of the bytecode. The proofs are meant to be produced by autonomous LLM agents.
...
* [evm-sail](https://github.com/frisitano/evm-sail): A formal and executable specification of the EVM written in [Sail](https://github.com/rems-project/sail), the ISA-specification language behind the official RISC-V model, structured as a state kernel with the EVM interpreter on top. The same artifact exports to Coq, Isabelle/HOL, HOL4 and Lean, and compiles to C to run the execution-spec-tests conformance suites.
    - Docs: [evm-sail specification, engineering notes and benchmarks](https://frisitano.github.io/evm-sail/).
* [EvmSemantics](https://github.com/powdr-labs/evm-semantics): A relational small-step and big-step semantics of the EVM in Lean 4, mirroring the structure of EVMYulLean but expressed as `Prop`-valued inductive relations, with a total executable interpreter proven sound against the relation.
...
* [yul-compiler](https://github.com/powdr-labs/yul-compiler): A verified compiler from a fragment of Yul to EVM bytecode in Lean 4, going through a labeled-assembly intermediate layer. Its correctness theorem is stated against [yul-semantics](https://github.com/powdr-labs/yul-semantics) and [EvmSemantics](https://github.com/powdr-labs/evm-semantics) as ordinary dependencies rather than re-encoded models, and compilation rejects what it cannot verify instead of emitting unverified code.
...
* [yul-semantics](https://github.com/powdr-labs/yul-semantics): A gas-free big-step relational semantics of Yul in Lean 4, parameterized over an abstract dialect, with a fuel-indexed executable interpreter proven adequate and determinism proven for the EVM dialect.
...
* [Verified zk(E)VM](https://github.com/Verified-zkEVM): A research effort accelerating the application of formal verification to zk(E)VMs in Lean ([verified-zkevm.org](https://verified-zkevm.org)).
	...
	- [evm-asm](https://github.com/Verified-zkEVM/evm-asm): An experimental verified macro assembler in Lean 4 over a RISC-V RV64IM backend, aiming at a zkEVM stateless block validator that carries a Lean-checked Hoare triple from block bytes to post-state root, keeping the compiler out of the trusted computing base.
